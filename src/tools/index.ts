...
export const lockRequirementsTool: ToolDefinition = {
  ...
  description:
    "Signal that requirements are understood and locked. Transitions to RED state.",
  inputSchema: {
    type: "object",
...
function handleStrReplace(params: any): unknown {
  ...
  if (!params.old_str || !content.includes(params.old_str)) {
    return {
      error: `String not found in ${params.path}: ${params.old_str?.slice(0, 50)}`,
    };
  }
  const count = content.split(params.old_str).length - 1;
  if (count > 1) {
    return {
      error: `Found ${count} matches for replacement text. Please provide more context to make a unique match.`,
    };
  }
...
export const strReplaceBasedEditTool: ToolDefinition = {
  ...
  execute: async (input: unknown) => {
    ...
    switch (params.command) {
      ...
      case "str_replace":
        return handleStrReplace(params);
      case "insert":
