...
describe('TASK mode end-to-end', () => {
  ...
  it('reads package.json and returns the project name', async () => {
    ...
    try {
      await runFSM(createTaskFSM(), 'task', 'read package.json and return the name field')
    } finally {
    ...
    expect(logs.some(l => l.includes('→ bash')) || logs.some(l => l.includes('→ str_replace_based_edit_tool'))).toBe(true)
