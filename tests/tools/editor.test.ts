...
describe('str_replace_based_edit_tool', () => {
  ...
  describe('create command', () => {
    ...
    it('creates parent directories if needed', async () => {
      const nestedFile = path.join(tmpDir, `groove-nested-${Date.now()}`, 'file.txt')
      const result = await strReplaceBasedEditTool.execute({
        command: 'create',
        path: nestedFile,
        file_text: 'nested',
  ...
  describe('str_replace command', () => {
    ...
    it('returns error when old_str is not found', async () => {
      ...
      expect(result).toMatchObject({ error: expect.stringContaining('String not found') })
    })
    ...
    it('returns error when old_str matches multiple times', async () => {
      ...
      expect(result).toMatchObject({ error: expect.stringContaining('3 matches') })
    })
  })
  ...
  describe('unknown command', () => {
    it('returns error for unsupported command', async () => {
      ...
      expect(result).toMatchObject({ error: expect.stringContaining('Unknown command') })
    })
  })
})
